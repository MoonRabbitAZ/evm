...
impl cumulus_pallet_jurisdiction_system::Config for Test {
	type SelfParaId = JurisdictionId;
	type Event = Event;
...
pub(crate) fn roll_to(n: u64) {
	while System::block_number() < n {
		...
		let inherent_data = {
			...
			let system_inherent_data = JurisdictionInherentData {
			...
			inherent_data
				.put_data(
					cumulus_primitives_jurisdiction_inherent::INHERENT_IDENTIFIER,
		...
		JurisdictionSystem::on_initialize(System::block_number());
		JurisdictionSystem::create_inherent(&inherent_data)
			.expect("got an inherent")
			.dispatch_bypass_filter(RawOrigin::None.into())
			.expect("dispatch succeeded");
		JurisdictionSystem::on_finalize(System::block_number());
