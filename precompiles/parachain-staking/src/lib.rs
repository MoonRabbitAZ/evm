...
//! Precompile to call jurisdiction-staking runtime methods via the EVM
...
/// A precompile to wrap the functionality from jurisdiction_staking.
///
/// EXAMPLE USECASE:
...
pub struct JurisdictionStakingWrapper<Runtime>(PhantomData<Runtime>);

impl<Runtime> Precompile for JurisdictionStakingWrapper<Runtime>
where
	Runtime: jurisdiction_staking::Config + pallet_evm::Config,
	Runtime::AccountId: From<H160>,
	BalanceOf<Runtime>: EvmData,
	Runtime::Call: Dispatchable<PostInfo = PostDispatchInfo> + GetDispatchInfo,
	<Runtime::Call as Dispatchable>::Origin: From<Option<Runtime::AccountId>>,
	Runtime::Call: From<jurisdiction_staking::Call<Runtime>>,
{
	fn execute(
...
impl<Runtime> JurisdictionStakingWrapper<Runtime>
where
	Runtime: jurisdiction_staking::Config + pallet_evm::Config,
	Runtime::AccountId: From<H160>,
	BalanceOf<Runtime>: EvmData,
	...
	Runtime::Call: From<jurisdiction_staking::Call<Runtime>>,
{
	...
	fn min_nomination(target_gas: Option<u64>) -> Result<PrecompileOutput, ExitError> {
		...
		let min_nomination: u128 = <<Runtime as jurisdiction_staking::Config>::MinNomination as Get<
			BalanceOf<Runtime>,
		>>::get()
		.try_into()
	...
	fn points(
		...
		// Fetch info.
		gasometer.record_cost(RuntimeHelper::<Runtime>::db_read_gas_cost())?;
		let points: u32 = jurisdiction_staking::Pallet::<Runtime>::points(round);
	...
	fn candidate_count(target_gas: Option<u64>) -> Result<PrecompileOutput, ExitError> {
		...
		// Fetch info.
		gasometer.record_cost(RuntimeHelper::<Runtime>::db_read_gas_cost())?;
		let candidate_count: u32 = <jurisdiction_staking::Pallet<Runtime>>::candidate_pool()
			.0
			.len() as u32;
	...
	fn collator_nomination_count(
		...
		gasometer.record_cost(RuntimeHelper::<Runtime>::db_read_gas_cost())?;
		let result =
			if let Some(state) = <jurisdiction_staking::Pallet<Runtime>>::collator_state2(&address) {
	...
	fn nominator_nomination_count(
		...
		gasometer.record_cost(RuntimeHelper::<Runtime>::db_read_gas_cost())?;
		let result =
			if let Some(state) = <jurisdiction_staking::Pallet<Runtime>>::nominator_state2(&address) {
	...
	fn is_nominator(
		...
		// Fetch info.
		gasometer.record_cost(RuntimeHelper::<Runtime>::db_read_gas_cost())?;
		let is_nominator = jurisdiction_staking::Pallet::<Runtime>::is_nominator(&address);
	...
	fn is_candidate(
		...
		// Fetch info.
		gasometer.record_cost(RuntimeHelper::<Runtime>::db_read_gas_cost())?;
		let is_candidate = jurisdiction_staking::Pallet::<Runtime>::is_candidate(&address);
	...
	fn is_selected_candidate(
		...
		// Fetch info.
		gasometer.record_cost(RuntimeHelper::<Runtime>::db_read_gas_cost())?;
		let is_selected = jurisdiction_staking::Pallet::<Runtime>::is_selected_candidate(&address);
	...
	fn join_candidates(
		...
		(
			...
			jurisdiction_staking::Call<Runtime>,
		),
		ExitError,
	> {
		// Read input.
		...
		let call =
			jurisdiction_staking::Call::<Runtime>::join_candidates(amount, collator_candidate_count);
	...
	> {
		...
		// Build call with origin.
		let origin = Runtime::AddressMapping::into_account_id(context.caller);
		let call = jurisdiction_staking::Call::<Runtime>::leave_candidates(collator_candidate_count);
	...
	> {
		// Build call with origin.
		let origin = Runtime::AddressMapping::into_account_id(context.caller);
		let call = jurisdiction_staking::Call::<Runtime>::go_offline();
	...
	> {
		// Build call with origin.
		let origin = Runtime::AddressMapping::into_account_id(context.caller);
		let call = jurisdiction_staking::Call::<Runtime>::go_online();
	...
	> {
		...
		// Build call with origin.
		let origin = Runtime::AddressMapping::into_account_id(context.caller);
		let call = jurisdiction_staking::Call::<Runtime>::candidate_bond_more(amount);
	...
	> {
		...
		// Build call with origin.
		let origin = Runtime::AddressMapping::into_account_id(context.caller);
		let call = jurisdiction_staking::Call::<Runtime>::candidate_bond_less(amount);
	...
	> {
		...
		// Build call with origin.
		let origin = Runtime::AddressMapping::into_account_id(context.caller);
		let call = jurisdiction_staking::Call::<Runtime>::nominate(
			collator,
			amount,
			collator_nomination_count,
			nominator_nomination_count,
	...
	> {
		...
		// Build call with origin.
		let origin = Runtime::AddressMapping::into_account_id(context.caller);
		let call = jurisdiction_staking::Call::<Runtime>::leave_nominators(nomination_count);
	...
	> {
		...
		// Build call with origin.
		let origin = Runtime::AddressMapping::into_account_id(context.caller);
		let call = jurisdiction_staking::Call::<Runtime>::revoke_nomination(collator);
	...
	> {
		...
		// Build call with origin.
		let origin = Runtime::AddressMapping::into_account_id(context.caller);
		let call = jurisdiction_staking::Call::<Runtime>::nominator_bond_more(collator, amount);
	...
	> {
		...
		// Build call with origin.
		let origin = Runtime::AddressMapping::into_account_id(context.caller);
		let call = jurisdiction_staking::Call::<Runtime>::nominator_bond_less(collator, amount);
