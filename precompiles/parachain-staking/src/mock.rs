...
use jurisdiction_staking::{InflationInfo, Range};
...
construct_runtime!(
		...
		UncheckedExtrinsic = UncheckedExtrinsic,
	{
		...
		JurisdictionStaking: jurisdiction_staking::{Pallet, Call, Storage, Config<T>, Event<T>},
	}
);
...
where
	...
	R: jurisdiction_staking::Config + pallet_evm::Config,
	R::AccountId: From<H160>,
	BalanceOf<R>: EvmData,
	R::Call: From<jurisdiction_staking::Call<R>>,
{
	fn execute(
		address: H160,
		input: &[u8],
		...
		match address {
			a if a == precompile_address() => Some(JurisdictionStakingWrapper::<R>::execute(
...
impl jurisdiction_staking::Config for Test {
	type Event = Event;
	type Currency = Balances;
...
impl ExtBuilder {
	...
	pub(crate) fn build(self) -> sp_io::TestExternalities {
		...
		jurisdiction_staking::GenesisConfig::<Test> {
			candidates: self.collators,
			nominations: self.nominations,
			inflation_config: self.inflation,
		}
		.assimilate_storage(&mut t)
		.expect("Jurisdiction Staking's storage can be assimilated");
...
pub(crate) fn set_points(round: u32, acc: TestAccount, pts: u32) {
	<jurisdiction_staking::Points<Test>>::mutate(round, |p| *p += pts);
	<jurisdiction_staking::AwardedPts<Test>>::mutate(round, acc, |p| *p += pts);
}
