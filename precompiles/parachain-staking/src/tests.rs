...
use crate::mock::{
	...
	JurisdictionStaking, Precompiles, TestAccount,
};
use crate::PrecompileOutput;
use frame_support::{assert_ok, dispatch::Dispatchable};
use pallet_evm::Call as EvmCall;
use pallet_evm::{ExitSucceed, PrecompileSet};
use jurisdiction_staking::Event as StakingEvent;
...
fn go_online_works() {
	ExtBuilder::default()
		...
		.build()
		.execute_with(|| {
			assert_ok!(JurisdictionStaking::go_offline(Origin::signed(
				TestAccount::Alice
			)));
...
fn nominate_works() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			assert!(JurisdictionStaking::is_nominator(&TestAccount::Bob));

			let expected: crate::mock::Event = StakingEvent::Nomination(
				TestAccount::Bob,
				1_000,
				TestAccount::Alice,
				jurisdiction_staking::NominatorAdded::AddedToTop { new_total: 2_000 },
			)
			.into();
			// Assert that the events vector contains the one expected
			assert!(events().contains(&expected));
