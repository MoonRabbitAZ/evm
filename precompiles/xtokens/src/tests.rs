...
fn junction_decoder_works() {
	ExtBuilder::default().build().execute_with(|| {
		let writer_output = EvmDataWriter::new()
			.write(JunctionWrapper::from(Junction::Jurisdiction(0)))
			.build();
		...
		assert_eq!(parsed, Junction::Jurisdiction(0));
...
fn network_id_decoder_works() {
	ExtBuilder::default().build().execute_with(|| {
		...
		assert_eq!(
			network_id_from_bytes(network_id_to_bytes(NetworkId::moonrabbit)),
			Ok(NetworkId::moonrabbit)
		);
	});
}
...
fn transfer_non_reserve_to_non_reserve_works() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			// We are transferring asset 1, which corresponds to another jurisdiction Id asset
			assert_eq!(
				Precompiles::execute(
					Precompile.into(),
					&EvmDataWriter::new()
	...
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			let asset_location = MultiLocation::new(
				1,
				Junctions::X2(Junction::Jurisdiction(2), Junction::GeneralIndex(5u128)),
			);
