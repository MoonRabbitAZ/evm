...
use cumulus_primitives_jurisdiction_inherent::JurisdictionInherentData;
use frame_support::{
	assert_ok,
...
pub use  mrevm_runtime::{
	currency::AAA, AccountId, AuthorInherent, Balance, Balances, Call, CrowdloanRewards, Ethereum,
	Event, Executive, FixedGasPrice, InflationInfo, JurisdictionStaking, Range, Runtime, System,
...
pub fn run_to_block(n: u32) {
	while System::block_number() < n {
		...
		JurisdictionStaking::on_initialize(System::block_number());
...
impl ExtBuilder {
	...
	pub fn build(self) -> sp_io::TestExternalities {
		...
		jurisdiction_staking::GenesisConfig::<Runtime> {
...
/// Mock the inherent that sets validation data in JurisdictionSystem, which
...
pub fn set_jurisdiction_inherent_data() {
	...
	let jurisdiction_inherent_data = JurisdictionInherentData {
		validation_data: vfp,
		relay_chain_state: relay_chain_state,
		downward_messages: Default::default(),
		horizontal_messages: Default::default(),
	};
	assert_ok!(Call::JurisdictionSystem(
		cumulus_pallet_jurisdiction_system::Call::<Runtime>::set_validation_data(
			jurisdiction_inherent_data
		)
	)
	.dispatch(inherent_origin()));
}
