...
//!  mrevm Runtime Api Integration Tests
...
use  mrevm_rpc_primitives_txpool::runtime_decl_for_TxPoolRuntimeApi::TxPoolRuntimeApi;
...
fn txpool_runtime_api_extrinsic_filter() {
	ExtBuilder::default().build().execute_with(|| {
		let non_eth_uxt = UncheckedExtrinsic::new_unsigned(
			pallet_balances::Call::<Runtime>::transfer(AccountId::from(BOB), 1 * AAA).into(),
		);
		let eth_uxt = uxt();
		let txpool = <Runtime as TxPoolRuntimeApi< mrevm_runtime::Block>>::extrinsic_filter(
