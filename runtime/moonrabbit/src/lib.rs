...
use cumulus_pallet_jurisdiction_system::RelaychainBlockNumberProvider;
...
pub use jurisdiction_staking::{InflationInfo, Range};
...
mod precompiles;
use precompiles:: mrevmPrecompiles;

#[cfg(any(feature = "std", test))]
pub use sp_runtime::BuildStorage;

pub type Precompiles =  mrevmPrecompiles<Runtime>;

/// AAA, the native token, uses 18 decimals of precision.
pub mod currency {
	use super::Balance;
	...
	pub const MICROAAA: Balance = 1_000_000_000_000;
	pub const MILLIAAA: Balance = 1_000_000_000_000_000;
	pub const AAA: Balance = 1_000_000_000_000_000_000;
	pub const KILOAAA: Balance = 1_000_000_000_000_000_000_000;

	pub const TRANSACTION_BYTE_FEE: Balance = 10 * MICROAAA;
	pub const STORAGE_BYTE_FEE: Balance = 100 * MICROAAA;

	pub const fn deposit(items: u32, bytes: u32) -> Balance {
		items as Balance * 1 * AAA + (bytes as Balance) * STORAGE_BYTE_FEE
	}
}
...
pub const VERSION: RuntimeVersion = RuntimeVersion {
	spec_name: create_runtime_str!(" mrevm"),
	impl_name: create_runtime_str!(" mrevm"),
	authoring_version: 3,
	spec_version: 0400,
	impl_version: 0,
...
parameter_types! {
	...
	/// These values are dictated by moonrabbit for the jurisdiction.
...
impl frame_system::Config for Runtime {
	...
	type OnSetCode = cumulus_pallet_jurisdiction_system::JurisdictionSetCode<Self>;
}
...
pub struct  mrevmGasWeightMapping;

impl pallet_evm::GasWeightMapping for  mrevmGasWeightMapping {
...
/// https://w3f-research.readthedocs.io/en/latest/moonrabbit/overview/2-token-economics.html#-2.-slow-adjusting-mechanism // editorconfig-checker-disable-line
///
...
impl pallet_evm::Config for Runtime {
	...
	type GasWeightMapping =  mrevmGasWeightMapping;
	...
	type Precompiles =  mrevmPrecompiles<Self>;
...
parameter_types! {
	...
	pub const MinimumDeposit: Balance = 4 * currency::AAA;
...
parameter_types! {
	...
	pub const ProposalBondMinimum: Balance = 1 * currency::AAA;
...
impl cumulus_pallet_jurisdiction_system::Config for Runtime {
	type Event = Event;
	type OnValidationData = ();
	type SelfParaId = JurisdictionInfo;
...
impl jurisdiction_info::Config for Runtime {}
...
parameter_types! {
	...
	/// Default percent of inflation set aside for jurisdiction bond every round
	pub const DefaultJurisdictionBondReservePercent: Percent = Percent::from_percent(30);
	/// Minimum stake required to become a collator is 1_000
	pub const MinCollatorStk: u128 = 1 * currency::KILOAAA;
	/// Minimum stake required to be reserved to be a candidate is 1_000
	pub const MinCollatorCandidateStk: u128 = 1 * currency::KILOAAA;
	/// Minimum stake required to be reserved to be a nominator is 5
	pub const MinNominatorStk: u128 = 5 * currency::AAA;
}
impl jurisdiction_staking::Config for Runtime {
	type Event = Event;
	type Currency = Balances;
	...
	type DefaultJurisdictionBondReservePercent = DefaultJurisdictionBondReservePercent;
	type MinCollatorStk = MinCollatorStk;
	type MinCollatorCandidateStk = MinCollatorCandidateStk;
	type MinNomination = MinNominatorStk;
	type MinNominatorStk = MinNominatorStk;
	type WeightInfo = jurisdiction_staking::weights::SubstrateWeight<Runtime>;
}
...
impl pallet_author_inherent::Config for Runtime {
	...
	type EventHandler = JurisdictionStaking;
...
impl pallet_author_slot_filter::Config for Runtime {
	...
	type PotentialAuthors = JurisdictionStaking;
}
...
parameter_types! {
	pub const DepositAmount: Balance = 100 * currency::AAA;
}
		...
		match self {
			...
			ProxyType::NonTransfer => {
				matches!(
					c,
					Call::System(..)
						| Call::Timestamp(..) | Call::JurisdictionStaking(..)
			...
			ProxyType::Staking => matches!(c, Call::JurisdictionStaking(..) | Call::Utility(..)),
...
/// Call filter expected to be used during Phase 3 of the  mrevm rollout
/// At least it was used in mrevm phase3
...
construct_runtime! {
		...
		UncheckedExtrinsic = UncheckedExtrinsic
	{
		...
		JurisdictionSystem: cumulus_pallet_jurisdiction_system::{Pallet, Call, Storage, Inherent, Event<T>},
		TransactionPayment: pallet_transaction_payment::{Pallet, Storage},
		JurisdictionInfo: jurisdiction_info::{Pallet, Storage, Config},
		EthereumChainId: pallet_ethereum_chain_id::{Pallet, Storage, Config},
		EVM: pallet_evm::{Pallet, Config, Call, Storage, Event<T>},
		Ethereum: pallet_ethereum::{Pallet, Call, Storage, Event, Config, ValidateUnsigned},
		JurisdictionStaking: jurisdiction_staking::{Pallet, Call, Storage, Event<T>, Config<T>},
...
// Check the timestamp and jurisdiction inherents
struct CheckInherents;

impl cumulus_pallet_jurisdiction_system::CheckInherents<Block> for CheckInherents {
	fn check_inherents(
		block: &Block,
		relay_state_proof: &cumulus_pallet_jurisdiction_system::RelayChainStateProof,
...
cumulus_pallet_jurisdiction_system::register_validate_block!(
	Runtime = Runtime,
