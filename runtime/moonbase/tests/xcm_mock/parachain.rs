...
use moonrabbit_core_primitives::BlockNumber as RelayBlockNumber;
use moonrabbit_jurisdiction::primitives::{Id as ParaId, Sibling};
use xcm::v1::{
	AssetId as XcmAssetId, Error as XcmError, ExecuteXcm,
	Junction::{PalletInstance, Jurisdiction},
...
use xcm_builder::{
	...
	RelayChainAsNative, SiblingJurisdictionAsNative, SiblingJurisdictionConvertsVia,
...
pub type AccountId =  mrevm_core_primitives::AccountId;
...
pub type LocationToAccountId = (
	...
	// Sibling jurisdiction origins convert to AccountId via the `ParaId::into`.
	SiblingJurisdictionConvertsVia<Sibling, AccountId>,
...
pub type XcmOriginToTransactDispatchOrigin = (
	...
	// Native converter for sibling Jurisdictions; will convert to a `SiblingPara` origin when
	// recognised.
	SiblingJurisdictionAsNative<cumulus_pallet_xcm::Origin, Origin>,
...
pub type XcmRouter = super::JurisdictionXcmRouter<MsgQueue>;
...
parameter_types! {
	...
	pub Ancestry: MultiLocation = Jurisdiction(MsgQueue::jurisdiction_id().into()).into();
		pub SelfReserve: MultiLocation = MultiLocation {
		parents:1,
		interior: Junctions::X2(
			Jurisdiction(MsgQueue::jurisdiction_id().into()),
...
impl Config for XcmConfig {
	...
	type SubscriptionService = moonrabbitXcm;
}
...
pub mod mock_msg_queue {
	...
	#[pallet::storage]
	#[pallet::getter(fn jurisdiction_id)]
	pub(super) type JurisdictionId<T: Config> = StorageValue<_, ParaId, ValueQuery>;
	...
	impl<T: Config> Pallet<T> {
		...
		fn handle_xcmp_message(
			...
			let (result, event) = match Xcm::<T::Call>::try_from(xcm) {
				Ok(xcm) => {
					let location = MultiLocation::new(1, Junctions::X1(Jurisdiction(sender.into())));
