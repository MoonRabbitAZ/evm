...
//! https://github.com/paritytech/cumulus/blob/master/rococo-jurisdictions/tests/purge_chain_works.rs
...
fn purge_chain_purges_relay_and_para() {
	fn run_node_and_stop() -> tempfile::TempDir {
		...
		let mut cmd = Command::new(cargo_bin(" mrevm"))
			.arg("-d")
			.arg(base_path.path())
			.arg("--chain")
			.arg("moonbase-dev")
...

	{
		...
		let status = Command::new(cargo_bin(" mrevm"))
			.args(&["purge-chain", "-d"])
			.arg(base_path.path())
			.arg("--chain")
			.arg("moonbase-dev")
		...
		// Make sure the jurisdiction data directory exists
...
fn builds_specs_based_on_mnemonic() {
	...
	let output = Command::new(cargo_bin(" mrevm"))
		.arg("build-spec")
		.arg("--dev")
		.arg("--mnemonic")
