...
use moonrabbit_jurisdiction::primitives::AccountIdConversion;
use moonrabbit_service::WestendChainSpec;
use sc_cli::{
	...
	Ok(match id {
		...
		"moonbase-alpha" | "alphanet" => {
			Box::new(chain_spec::moonbase::ChainSpec::from_json_bytes(
				&include_bytes!("../../../specs/alphanet/jurisdiction-embedded-specs-v8.json")[..],
			)?)
		}
		...
		// mrevm networks
		"mrevm" => Box::new(chain_spec::mrevm::ChainSpec::from_json_bytes(
			&include_bytes!("../../../specs/mrevm/jurisdiction-embedded-specs.json")[..],
		)?),
		"mrevm-dev" => Box::new(chain_spec::mrevm::development_chain_spec(None, None)),
		"mrevm-local" => Box::new(chain_spec::mrevm::get_chain_spec(para_id)),

		//  mrevm networks
		" mrevm" => {
			return Err(
				"You chosen the  mrevm mainnet spec. This network is not yet available.".into(),
			);
			// Box::new(chain_spec::mrevm::ChainSpec::from_json_bytes(
			// 	&include_bytes!("../../../specs/ mrevm.json")[..],
			// )?)
		}
		" mrevm-dev" => Box::new(chain_spec:: mrevm::development_chain_spec(None, None)),
		" mrevm-local" => Box::new(chain_spec:: mrevm::get_chain_spec(para_id)),
		...
		// ` mrevm-custom.json` uses the  mrevm runtime.
		...
		// TODO explore CLI options to make this nicer. eg `--force-mrevm-runtime`
		path => {
			...
			} else if run_cmd.force_mrevm || starts_with("mrevm") {
				Box::new(chain_spec::mrevm::ChainSpec::from_json_file(path)?)
			} else {
				Box::new(chain_spec:: mrevm::ChainSpec::from_json_file(path)?)
			}
		}
...
impl SubstrateCli for Cli {
	fn impl_name() -> String {
		" mrevm Jurisdiction Collator".into()
	}
	...
	fn description() -> String {
		format!(
			"Moonbase Jurisdiction Collator\n\nThe command-line arguments provided first will be \
		passed to the jurisdiction node, while the arguments provided after -- will be passed \
		to the relaychain node.\n\n\
		{} [jurisdiction-args] -- [relaychain-args]",
	...
	fn support_url() -> String {
		"https://github.com/moonrabbit/ mrevm/issues/new".into()
	}
	...
	fn load_spec(&self, id: &str) -> std::result::Result<Box<dyn sc_service::ChainSpec>, String> {
		load_spec(id, self.run.jurisdiction_id.unwrap_or(1000).into(), &self.run)
	}

	fn native_runtime_version(spec: &Box<dyn sc_service::ChainSpec>) -> &'static RuntimeVersion {
		if spec.is_moonbase() {
			return &service::moonbase_runtime::VERSION;
		} else if spec.is_mrevm() {
			return &service::mrevm_runtime::VERSION;
		} else {
			return &service:: mrevm_runtime::VERSION;
		}
	}
...
impl SubstrateCli for RelayChainCli {
	...
	fn description() -> String {
		" mrevm Jurisdiction Collator\n\nThe command-line arguments provided first will be \
		passed to the jurisdiction node, while the arguments provided after -- will be passed \
		to the relaychain node.\n\n\
		jurisdiction-collator [jurisdiction-args] -- [relaychain-args]"
			.into()
	}
	...
	fn load_spec(&self, id: &str) -> std::result::Result<Box<dyn sc_service::ChainSpec>, String> {
		match id {
			...
			// If we are not using a  mrevm-centric pre-baked relay spec, then fall back to the
			// moonrabbit service to interpret the id.
			_ => moonrabbit_cli::Cli::from_iter([RelayChainCli::executable_name()].iter())
				.load_spec(id),
		}
	}

	fn native_runtime_version(chain_spec: &Box<dyn ChainSpec>) -> &'static RuntimeVersion {
		moonrabbit_cli::Cli::native_runtime_version(chain_spec)
	}
}
	...
	match &cli.subcommand {
		Some(Subcommand::BuildSpec(params)) => {
			...
			runner.sync_run(|config| {
				if params.mnemonic.is_some() || params.accounts.is_some() {
					if config.chain_spec.is_ mrevm() {
						params.base.run(
							Box::new(chain_spec:: mrevm::development_chain_spec(
		...
		Some(Subcommand::PurgeChain(cmd)) => {
			...
			runner.sync_run(|config| {
				...
				let moonrabbit_cli = RelayChainCli::new(
					&config,
				...
				let moonrabbit_config = SubstrateCli::create_configuration(
					&moonrabbit_cli,
					&moonrabbit_cli,
				...
				cmd.run(config, moonrabbit_config)
			})
		}
		...
		Some(Subcommand::ExportGenesisState(params)) => {
			...
			let chain_spec = load_spec(
				...
				params.jurisdiction_id.unwrap_or(1000).into(),
				&cli.run,
			)?;
			let output_buf = if chain_spec.is_ mrevm() {
				let block: service:: mrevm_runtime::Block =
				...
				output_buf
			} else if chain_spec.is_mrevm() {
				let block: service::mrevm_runtime::Block = generate_genesis_block(&chain_spec)?;
				let raw_header = block.header().encode();
				let output_buf = if params.raw {
					raw_header
				} else {
		...
		Some(Subcommand::Benchmark(cmd)) => {
			if cfg!(feature = "runtime-benchmarks") {
				...
				if chain_spec.is_ mrevm() {
					return runner.sync_run(|config| {
						cmd.run::<service:: mrevm_runtime::Block, service:: mrevmExecutor>(
							config,
						)
					});
				} else if chain_spec.is_mrevm() {
		...
		None => {
			...
			runner.run_node_until_exit(|config| async move {
				...
				let id = ParaId::from(cli.run.jurisdiction_id.clone().or(para_id).unwrap_or(1000));
				...
				// Otherwise continue with the normal jurisdiction node.
				...
				let jurisdiction_account =
					AccountIdConversion::<moonrabbit_primitives::v0::AccountId>::into_account(&id);

				let genesis_state = if config.chain_spec.is_ mrevm() {
					let block: service:: mrevm_runtime::Block =
				...
				let moonrabbit_config =
					SubstrateCli::create_configuration(&moonrabbit_cli, &moonrabbit_cli, task_executor)
						.map_err(|err| format!("Relay chain argument error: {}", err))?;

				info!("Jurisdiction id: {:?}", id);
				info!("Jurisdiction Account: {}", jurisdiction_account);
				info!("Jurisdiction genesis state: {}", genesis_state);

				if config.chain_spec.is_ mrevm() {
					service::start_node::<
						service:: mrevm_runtime::RuntimeApi,
						service:: mrevmExecutor,
					>(config, moonrabbit_config, id, rpc_config)
					.await
					.map(|r| r.0)
					.map_err(Into::into)
				} else if config.chain_spec.is_mrevm() {
...
impl CliConfiguration<Self> for RelayChainCli {
	...
	fn init<C: SubstrateCli>(&self) -> Result<()> {
		unreachable!("moonrabbitCli is never initialized; qed");
	}
