...
//!  mrevm CLI Library. Built with structopt
//!
//! This module defines the  mrevm node's Command Line Interface (CLI)
...
pub enum Subcommand {
	/// Export the genesis state of the jurisdiction.
	#[structopt(name = "export-genesis-state")]
	ExportGenesisState(ExportGenesisStateCommand),

	/// Export the genesis wasm of the jurisdiction.
...
/// Command for exporting the genesis state of the jurisdiction
#[derive(Debug, StructOpt)]
pub struct ExportGenesisStateCommand {
	...
	/// Id of the jurisdiction this state is for.
	#[structopt(long)]
	pub jurisdiction_id: Option<u32>,
...
pub struct RunCmd {
	...
	/// Force using mrevm native runtime.
	#[structopt(long = "force-mrevm")]
	pub force_mrevm: bool,
}
...
pub struct RelayChainCli {
	...
	pub base: moonrabbit_cli::RunCmd,
...
impl RelayChainCli {
	...
	pub fn new<'a>(
		...
		let base_path = para_config
			.base_path
			.as_ref()
			.map(|x| x.path().join("moonrabbit"));
		Self {
			base_path,
			chain_id,
			base: moonrabbit_cli::RunCmd::from_iter(relay_chain_args),
		}
	}
}
