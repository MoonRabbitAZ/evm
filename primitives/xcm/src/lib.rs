...
use xcm::v1::{
	...
	Junction::{AccountKey20, Jurisdiction},
	Junctions::*,
	...
	fn reserve(&self) -> Option<MultiLocation> {
		if let xcmAssetId::Concrete(location) = self.id.clone() {
			...
			match (parents, first_interior.clone()) {
				(0, Some(Jurisdiction(id))) => Some(MultiLocation::new(0, X1(Jurisdiction(id.clone())))),
				(1, Some(Jurisdiction(id))) => Some(MultiLocation::new(1, X1(Jurisdiction(id.clone())))),
