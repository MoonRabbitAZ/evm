...
benchmarks! {
	...
	set_jurisdiction_bond_account {
		let jurisdiction_bond_account: T::AccountId = account("TEST", 0u32, USER_SEED);
	}: _(RawOrigin::Root, jurisdiction_bond_account.clone())
	verify {
		assert_eq!(Pallet::<T>::jurisdiction_bond_info().account, jurisdiction_bond_account);
	}

	set_jurisdiction_bond_reserve_percent {
	}: _(RawOrigin::Root, Percent::from_percent(33))
	verify {
		assert_eq!(Pallet::<T>::jurisdiction_bond_info().percent, Percent::from_percent(33));
	}
...
mod tests {
	...
	#[test]
	fn bench_set_jurisdiction_bond_account() {
		new_test_ext().execute_with(|| {
			assert_ok!(test_benchmark_set_jurisdiction_bond_account::<Test>());
		});
	}

	#[test]
	fn bench_set_jurisdiction_bond_reserve_percent() {
		new_test_ext().execute_with(|| {
			assert_ok!(test_benchmark_set_jurisdiction_bond_reserve_percent::<Test>());
		});
	}
