...
fn invalid_monetary_origin_fails() {
	ExtBuilder::default().build().execute_with(|| {
		...
		assert_noop!(
			Stake::set_jurisdiction_bond_account(Origin::signed(45), 11),
			sp_runtime::DispatchError::BadOrigin
		);
		assert_noop!(
			Stake::set_jurisdiction_bond_reserve_percent(Origin::signed(45), Percent::from_percent(2)),
			sp_runtime::DispatchError::BadOrigin
		);
	});
}
...
// SET jurisdiction BOND ACCOUNT

#[test]
fn set_jurisdiction_bond_account_event_emits_correctly() {
	ExtBuilder::default().build().execute_with(|| {
		assert_ok!(Stake::set_jurisdiction_bond_account(Origin::root(), 11));
		assert_eq!(
			last_event(),
			MetaEvent::Stake(Event::JurisdictionBondAccountSet(0, 11))
		);
	});
}

#[test]
fn set_jurisdiction_bond_account_storage_updates_correctly() {
	ExtBuilder::default().build().execute_with(|| {
		assert_eq!(Stake::jurisdiction_bond_info().account, 0);
		assert_ok!(Stake::set_jurisdiction_bond_account(Origin::root(), 11));
		assert_eq!(Stake::jurisdiction_bond_info().account, 11);
	});
}

// SET jurisdiction BOND RESERVE PERCENT

#[test]
fn set_jurisdiction_bond_reserve_percent_event_emits_correctly() {
	ExtBuilder::default().build().execute_with(|| {
		assert_ok!(Stake::set_jurisdiction_bond_reserve_percent(
			Origin::root(),
			Percent::from_percent(50)
		));
		assert_eq!(
			last_event(),
			MetaEvent::Stake(Event::JurisdictionBondReservePercentSet(
...
#[test]
fn set_jurisdiction_bond_reserve_percent_storage_updates_correctly() {
	ExtBuilder::default().build().execute_with(|| {
		assert_eq!(
			Stake::jurisdiction_bond_info().percent,
			Percent::from_percent(30)
		);
		assert_ok!(Stake::set_jurisdiction_bond_reserve_percent(
			Origin::root(),
...
#[test]
fn cannot_set_same_jurisdiction_bond_reserve_percent() {
	ExtBuilder::default().build().execute_with(|| {
		assert_noop!(
			Stake::set_jurisdiction_bond_reserve_percent(Origin::root(), Percent::from_percent(30)),
			Error::<Test>::NoWritingSameValue
		);
	});
}
...
#[test]
fn jurisdiction_bond_inflation_reserve_matches_config() {
	ExtBuilder::default()
		.with_balances(vec![
			(1, 100),
			(2, 100),
		...
		.execute_with(|| {
			...
			// set jurisdiction bond account so DefaultJurisdictionBondReservePercent = 30% of inflation
			// is allocated to this account hereafter
			assert_ok!(Stake::set_jurisdiction_bond_account(Origin::root(), 11));
			roll_to(8);
			// chooses top TotalSelectedCandidates (5), in order
			let mut expected = vec![
				Event::JurisdictionBondAccountSet(0, 11),
			...
			let mut new = vec![
				...
				Event::ReservedForJurisdictionBond(11, 15),
				Event::Rewarded(1, 18),
				Event::Rewarded(7, 6),
				Event::Rewarded(10, 6),
				Event::Rewarded(6, 6),
			...
			let mut new2 = vec![
				...
				Event::ReservedForJurisdictionBond(11, 16),
				Event::Rewarded(1, 19),
				Event::Rewarded(7, 6),
				Event::Rewarded(10, 6),
				Event::Rewarded(6, 6),
			...
			// keep paying 6
			let mut new3 = vec![
				Event::JurisdictionBondReservePercentSet(
					Percent::from_percent(30),
					Percent::from_percent(50),
				),
				Event::ReservedForJurisdictionBond(11, 29),
				Event::Rewarded(1, 15),
				Event::Rewarded(7, 5),
				Event::Rewarded(10, 5),
				Event::Rewarded(6, 5),
			...
			// no more paying 6
			let mut new4 = vec![
				Event::ReservedForJurisdictionBond(11, 30),
				Event::Rewarded(1, 18),
				Event::Rewarded(7, 6),
				Event::Rewarded(10, 6),
			...
			let mut new5 = vec![
				...
				Event::ReservedForJurisdictionBond(11, 32),
				Event::Rewarded(1, 19),
				Event::Rewarded(7, 6),
				Event::Rewarded(10, 6),
			...
			let mut new6 = vec![
				Event::ReservedForJurisdictionBond(11, 33),
				Event::Rewarded(1, 20),
				Event::Rewarded(7, 7),
				Event::Rewarded(10, 7),
			...
			let mut new7 = vec![
				Event::ReservedForJurisdictionBond(11, 35),
				Event::Rewarded(1, 18),
				Event::Rewarded(7, 6),
				Event::Rewarded(8, 6),
				Event::Rewarded(10, 6),
