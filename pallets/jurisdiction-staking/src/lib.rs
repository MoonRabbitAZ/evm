...
pub mod pallet {
	...
	/// Pallet for jurisdiction staking
	#[pallet::pallet]
	...
	pub struct JurisdictionBondConfig<AccountId> {
		/// Account which receives funds intended for jurisdiction bond
		pub account: AccountId,
		/// Percent of inflation set aside for jurisdiction bond account
		pub percent: Percent,
	}
	impl<A: Default> Default for JurisdictionBondConfig<A> {
		fn default() -> JurisdictionBondConfig<A> {
			JurisdictionBondConfig {
				account: A::default(),
	...
	pub trait Config: frame_system::Config {
		...
		/// Default percent of inflation set aside for jurisdiction bond account
		#[pallet::constant]
		type DefaultJurisdictionBondReservePercent: Get<Percent>;
	...
	pub enum Event<T: Config> {
		...
		/// Transferred to account which holds funds reserved for jurisdiction bond
		ReservedForJurisdictionBond(T::AccountId, BalanceOf<T>),
		/// Account (re)set for jurisdiction bond treasury [old, new]
		JurisdictionBondAccountSet(T::AccountId, T::AccountId),
		/// Percent of inflation reserved for jurisdiction bond (re)set [old, new]
		JurisdictionBondReservePercentSet(Percent, Percent),
	...
	#[pallet::storage]
	#[pallet::getter(fn jurisdiction_bond_info)]
	/// Jurisdiction bond config info { account, percent_of_inflation }
	type JurisdictionBondInfo<T: Config> =
		StorageValue<_, JurisdictionBondConfig<T::AccountId>, ValueQuery>;
	...
	impl<T: Config> GenesisBuild<T> for GenesisConfig<T> {
		fn build(&self) {
			...
			// Set jurisdiction bond config to default config
			<JurisdictionBondInfo<T>>::put(JurisdictionBondConfig {
				// must be set soon; if not => due inflation will be sent to collators/nominators
				account: T::AccountId::default(),
				percent: T::DefaultJurisdictionBondReservePercent::get(),
			});
	...
	impl<T: Config> Pallet<T> {
		...
		/// Set the account that will hold funds set aside for jurisdiction bond
		#[pallet::weight(<T as Config>::WeightInfo::set_jurisdiction_bond_account())]
		pub fn set_jurisdiction_bond_account(
			origin: OriginFor<T>,
			new: T::AccountId,
		) -> DispatchResultWithPostInfo {
			T::MonetaryGovernanceOrigin::ensure_origin(origin)?;
			let JurisdictionBondConfig {
				account: old,
				percent,
			} = <JurisdictionBondInfo<T>>::get();
			ensure!(old != new, Error::<T>::NoWritingSameValue);
			<JurisdictionBondInfo<T>>::put(JurisdictionBondConfig {
				account: new.clone(),
				percent,
			});
			Self::deposit_event(Event::JurisdictionBondAccountSet(old, new));
			Ok(().into())
		}
		/// Set the percent of inflation set aside for jurisdiction bond
		#[pallet::weight(<T as Config>::WeightInfo::set_jurisdiction_bond_reserve_percent())]
		pub fn set_jurisdiction_bond_reserve_percent(
			origin: OriginFor<T>,
			new: Percent,
		) -> DispatchResultWithPostInfo {
			T::MonetaryGovernanceOrigin::ensure_origin(origin)?;
			...
			Self::deposit_event(Event::JurisdictionBondReservePercentSet(old, new));
			Ok(().into())
		}
	...
	impl<T: Config> Pallet<T> {
		...
		fn pay_stakers(next: RoundIndex) {
			...
			// reserve portion of issuance for jurisdiction bond account
			let bond_config = <JurisdictionBondInfo<T>>::get();
			let jurisdiction_bond_reserve = bond_config.percent * issuance;
			if let Ok(imb) =
				T::Currency::deposit_into_existing(&bond_config.account, jurisdiction_bond_reserve)
			{
				// update round issuance iff transfer succeeds
				issuance -= imb.peek();
				Self::deposit_event(Event::ReservedForJurisdictionBond(
					bond_config.account,
					imb.peek(),
				));
			}
